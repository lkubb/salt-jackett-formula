...
We will see a quick setup to configure the ``jackett`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``jackett`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``jackett`` formulas under ``/srv/salt/jackett/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/jackett/parameters/dns:domain/
    ln -s dns:domain /srv/salt/jackett/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/jackett/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/jackett/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``jackett`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``jackett`` formulas under ``/srv/salt/jackett/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/jackett/parameters/roles
...
We create a configuration for the role ``jackett/server`` in ``/srv/salt/jackett/parameters/roles/jackett/server.yaml``:
...
We create another configuration for the role ``jackett/client`` in ``/srv/salt/jackett/parameters/roles/jackett/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles jackett/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles jackett/client
...
.. code-block:: console
    ...
    mv /srv/salt/jackett/parameters/dns:domain/  '/srv/salt/jackett/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-jackett-as-json:
      test.nop:
        - name: {{ jackett | json }}
...
Here is an example based on `template-formula/jackett/config/file.sls`_:
    ...
    jackett-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='jackett-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ jackett.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``jackett`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ jackett['winner'] }}
...
.. _template-formula/jackett/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/jackett/config/file.sls
